...
class StudentList extends Component {
  ...
  async componentDidMount() {
    let [response, resp_2] = await Promise.all([
      Axios.get(API_URL["student_list"]),
      Axios.get(API_URL["all_activities"]),
    ]);
  ...
  name_filter = () => {
    this.setState((st) => ({
      student_list: st.student_list.filter(
        (s) =>
          s.username.toLowerCase().startsWith(st.search_text.toLowerCase()) ||
          s.name.toLowerCase().startsWith(st.search_text.toLowerCase())
      ),
    }));
  };
              ...
              <div className="search row">
                <div class="form-group col-sm-8">
                  <div className="row">
                    <div className="col-sm-1 d-flex justify-content-center align-items-center p-0">
                      <i
                        style={{ fontSize: "19px", color: "#2C3E50" }}
                ...
                <div className="col-sm-1">
                  <button
                    ...
                    data-target="#student_filter"
                    aria-expanded="false"
                    aria-controls="student_filter"
                  >
                    {" "}
